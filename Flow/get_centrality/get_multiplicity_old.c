...
void get_multiplicity(TString inFileName , TString outFileName , TString dcaFileName)
{
    ...
    for (int event = 0; event < n_events; ++event)
    {
		...
		if (TMath::Abs(mpdtrack->GetDCAX()) >= f_dca[0][pt_bin][eta_bin]->GetParameter(2)*2) continue;
		if (TMath::Abs(mpdtrack->GetDCAY()) >= f_dca[1][pt_bin][eta_bin]->GetParameter(2)*2) continue;
		if (TMath::Abs(mpdtrack->GetDCAZ()) >= f_dca[2][pt_bin][eta_bin]->GetParameter(2)*2) continue;
